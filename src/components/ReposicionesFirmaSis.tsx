...
export function ReposicionesFirmaSis({ onPendientesChange }: { onPendientesChange?: (cantidad: number) => void }) {
  ...
  useEffect(() => { onPendientesChange?.(solicitudes.filter((solicitud) => solicitud.estado === "pendiente").length); }, [onPendientesChange, solicitudes]);
  ...
  return <section className="mt-10 border-t border-slate-200 pt-8 dark:border-slate-800">
    <div className="mb-6 flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl border border-violet-200 bg-violet-50 text-violet-700 dark:border-violet-900 dark:bg-violet-950 dark:text-violet-300"><KeyRound size={19} /></span><div><h2 className="font-heading text-xl font-bold text-slate-900 dark:text-slate-100">Reposiciones de firma médica</h2><p className="text-xs text-slate-500">Solicitudes de llave SIS y respaldo privado para Administración.</p></div></div><button onClick={() => void cargar()} className="rounded-lg border border-slate-300 px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800">Actualizar</button></div>
