...
// sin ingreso; se conservan como opciones para no perder esa semántica.
export const SERVICIOS_INGRESO_REFERIDO = [
  ...SERVICIOS_INGRESO.filter((s) => s !== "Otro"),
  "Alta (no ingresa)",
...
export const siNo = (v: boolean) => (v ? "SI" : "NO");

// Lista de procedimientos → celda del Excel ("NO" cuando no hubo).
export const procsACelda = (procs: string[]) =>
  procs.length ? procs.map((p) => p.toUpperCase()).join(", ") : "NO";

// Diagnósticos CIE → celda ("DESCRIPCION (COD) // DESCRIPCION (COD)").
export const diagnosticosACelda = (dx: { codigo: string; descripcion: string }[]) =>
  dx.map((d) => `${d.descripcion.toUpperCase()}${d.codigo ? ` (${d.codigo})` : ""}`).join(" // ");
