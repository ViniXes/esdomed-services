...
export function calcularIndicadoresCuidadosCriticos(
  ...
  return [
    ...
    indicador(27, conteos.esperadoApache > 0 ? razon(conteos.fallecidos, conteos.esperadoApache) : null, conteos.fallecidos, conteos.esperadoApache, "pendiente_formula", "Formula preliminar: muertes observadas / muertes esperadas por MORTALIDAD II. Requiere validacion."),
...
export function calcularDatosBaseCuidadosCriticos(
  ...
  return [
    ...
    row("N. de muertes hospitalarias esperadas de acuerdo a escala de severidad de pacientes", conteos.map(item => numeroVista(item.esperadoApache)), "Actualmente se calcula como suma de MORTALIDAD II (%) / 100; requiere validacion de formula final."),
