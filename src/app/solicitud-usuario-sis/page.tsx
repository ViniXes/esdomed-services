...
import { CARGOS_USUARIO_SIS, ESPECIALIDADES_SIS, RESPUESTAS_SI_NO } from "@/lib/solicitudesUsuarioSis";
...
const EMPTY = {
  nombre: "", dui: "", correo: "", telefono: "", cargo: "", otroCargo: "", numeroJunta: "", yaTuvoUsuario: "",
          ...
          {enviada ? (
            ...
            <>
              ...
              <form onSubmit={enviar} className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-6">
                ...
                <Campo label="Nombre completo" className="lg:col-span-6"><input value={form.nombre} onChange={(e) => set("nombre", e.target.value.toUpperCase())} required autoComplete="name" className={inputCls} placeholder="NOMBRE COMPLETO" /></Campo>
                <Campo label="DUI" className="lg:col-span-2"><input value={form.dui} onChange={(e) => set("dui", normalizarDui(e.target.value))} required inputMode="numeric" className={inputCls} placeholder="00000000-0" /></Campo>
                ...
                <Campo label={form.cargo === "medico" ? "Número de junta médica" : "Junta / registro profesional (si aplica)"} className="lg:col-span-3"><input value={form.numeroJunta} onChange={(e) => set("numeroJunta", e.target.value.toUpperCase())} required={form.cargo === "medico"} className={inputCls} placeholder="Ej. 19711" /></Campo>
                ...
                <Campo label="Servicio al que será asignado" className={form.especialidad === "Otra" ? "lg:col-span-6" : "lg:col-span-3"}><input value={form.servicio} onChange={(e) => set("servicio", e.target.value)} required className={inputCls} placeholder="Ej. Medicina interna, UCI, Radiología" /></Campo>
                <Campo label="Autoriza" className="lg:col-span-3"><input value={form.autorizadoPor} onChange={(e) => set("autorizadoPor", e.target.value.toUpperCase())} required className={inputCls} placeholder="Nombre de jefatura responsable" /></Campo>
