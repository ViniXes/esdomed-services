          ...
          {enviada || vistaPrevia ? (
            ...
            <>
              ...
              <form onSubmit={enviar} className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-6">
                ...
                {esMedicoInterno ? <div className="lg:col-span-3 rounded-xl border border-cyan-200 bg-cyan-50 px-3 py-2.5 text-sm text-cyan-800 dark:border-cyan-900 dark:bg-cyan-950 dark:text-cyan-200"><p className="font-medium">Especialidad no aplica</p><p className="mt-0.5 text-xs">Selecciona únicamente el servicio asignado.</p></div> : <Campo label="Especialidad con la que trabajará (si aplica)" className="lg:col-span-3"><select value={form.especialidad} onChange={(e) => set("especialidad", e.target.value)} className={inputCls}><option value="">Seleccionar especialidad...</option>{ESPECIALIDADES_SIS.map((especialidad) => <option key={especialidad} value={especialidad}>{especialidad}</option>)}</select></Campo>}
