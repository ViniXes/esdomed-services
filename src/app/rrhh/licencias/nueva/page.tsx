...
import { CATEGORIAS, CATEGORIAS_ORDEN, metaCategoria, BOLSA_LABEL } from "@/lib/rrhh/catalogo";
import { evaluarLicencia } from "@/lib/rrhh/saldos";
...
export default function NuevaLicenciaPage() {
  ...
  const evaluacion = useMemo(() => {
    ...
    return evaluarLicencia({
      categoria,
      fechaInicial: parseDateInput(fechaInicial),
      fechaFinal: parseDateInput(fechaFinal),
      empleado,
      licenciasExistentes: licenciasEmpleado,
    });
  }, [empleado, categoria, fechaInicial, fechaFinal, licenciasEmpleado]);

  const fechasInvalidas = parseDateInput(fechaFinal) < parseDateInput(fechaInicial);

  const guardar = async () => {
    if (!user || !profile || !empleado || !evaluacion) return;
    setError(null);

    if (fechasInvalidas) { setError("La fecha final no puede ser anterior a la inicial."); return; }
    if (evaluacion.dias <= 0) { setError("El periodo debe ser de al menos un día."); return; }
    ...
    try {
      const data: Record<string, unknown> = {
        ...
        fechaInicial: Timestamp.fromDate(parseDateInput(fechaInicial)),
        fechaFinal: Timestamp.fromDate(parseDateInput(fechaFinal)),
        dias: evaluacion.dias,
        anio: evaluacion.anio,
        diasConGoce: evaluacion.diasConGoce,
        diasSinGoce: evaluacion.diasSinGoce,
  ...
  return (
    <div className="p-4 md:p-6 max-w-3xl mx-auto space-y-5">
      ...
      {empleado && evaluacion && !fechasInvalidas && evaluacion.dias > 0 && (
      ...
      {empleado && (
        <div>
          ...
          <button
            onClick={guardar}
            disabled={guardando || !!evaluacion?.bloqueado || fechasInvalidas}
...
function EvaluacionPanel({
  ...
  const restante = Math.max(0, ev.tope - ev.usadoPrevio - ev.dias);
  return (
    <section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 space-y-3">
      ...
      {!ev.descuentaSaldo ? (
        <p className="text-sm text-slate-500">
          Esta licencia ({ev.dias} día{ev.dias === 1 ? "" : "s"}) no descuenta de las bolsas de saldo.
        </p>
      ) : (
        <>
          <div className="grid grid-cols-4 gap-2 text-center">
            <Mini label="Días" value={ev.dias} />
            <Mini label="Tope" value={ev.tope} />
            <Mini label="Usado" value={ev.usadoPrevio} />
            <Mini label="Quedará" value={restante} accent={restante === 0} />
          </div>
          ...
          {ev.bloqueado ? (
            ...
            <>
              <div className="flex items-start gap-2 bg-amber-50 dark:bg-amber-950 border border-amber-200 dark:border-amber-900 rounded-lg px-3 py-2.5 text-sm text-amber-800 dark:text-amber-300">
                ...
                <span>
                  {ev.mensaje} <strong>{ev.diasConGoce} con goce</strong> · <strong>{ev.diasSinGoce} sin goce</strong>.
                </span>
              </div>
              <div>
            ...
            <p className="flex items-center gap-1.5 text-sm text-emerald-700 dark:text-emerald-400">
              <CheckCircle2 size={15} /> Dentro del tope. {ev.diasConGoce > 0 && `${ev.diasConGoce} con goce`}{ev.diasSinGoce > 0 && ` · ${ev.diasSinGoce} sin goce`}.
            </p>
          )}
        </>
      )}
...
function Mini({ label, value, accent }: { label: string; value: number; accent?: boolean }) {
  return (
    <div className="bg-slate-50 dark:bg-slate-800/60 rounded-lg py-2">
      <p className={`text-lg font-bold tabular-nums ${accent ? "text-amber-600 dark:text-amber-400" : "text-slate-900 dark:text-slate-100"}`}>{value}</p>
