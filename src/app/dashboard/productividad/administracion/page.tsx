    ...
    <div className="mx-auto max-w-[1600px] space-y-5 p-4 md:p-6">
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
        <div>
          ...
          <p className="mt-0.5 text-xs text-slate-500">Creaciones atribuidas de forma permanente al administrador que las realizó.</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Resumen icon={ShieldCheck} etiqueta="Administradores con producción" valor={porAdministrador.length} tono="text-indigo-600 dark:text-indigo-300" />
        <Resumen icon={ClipboardCheck} etiqueta="Día con más creaciones" valor={Math.max(0, ...porDia.map((dato) => dato.valor))} tono="text-emerald-600 dark:text-emerald-300" />
      </div>
      ...
      {cargando ? <div className="flex justify-center py-20"><div className="h-7 w-7 animate-spin rounded-full border-2 border-blue-500 border-t-transparent" /></div> : <>
        <div className="grid gap-5 xl:grid-cols-2">
          ...
          <GraficoPastel titulo="Participación por administrador" datos={porAdministrador} />
        </div>
        <GraficoBarrasHorizontalCorto titulo="Creaciones por día" datos={porDia} color="#4f46e5" />
