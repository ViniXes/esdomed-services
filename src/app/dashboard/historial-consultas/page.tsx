...
const CRITERIO_LABEL: Record<CriterioConsultaPaciente, string> = {
...
export default function HistorialConsultasPage() {
  ...
  const [criterio, setCriterio] = useState<"todos" | CriterioConsultaPaciente>("todos");
  ...
  const filtrados = useMemo(() => {
    ...
    return registros.filter(r => {
      ...
      if (criterio !== "todos" && r.criterio !== criterio) return false;
      ...
      return [
        ...
        r.usuarioJvpm,
        r.termino,
        r.expedienteConsultado,
        r.pacienteNombre,
        r.servicioActual,
      ].some(v => String(v ?? "").toLowerCase().includes(q));
    });
  }, [busqueda, desde, hasta, criterio, registros]);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Auditoría de fichas de paciente consultadas (médicos, trabajo social y psicología)
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_150px_150px_150px_auto] gap-2">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Buscar usuario, expediente o paciente..."
          ...
          <button
            onClick={() => { setBusqueda(""); setDesde(""); setHasta(""); setCriterio("todos"); }}
              ...
              <table className="w-full text-sm min-w-[960px]">
                <thead>
                  <tr className="border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
                    {["Fecha", "Usuario", "Criterio / término", "Paciente", "Ingreso activo", "Estancias"].map(h => (
                  ...
                  {filtrados.map(r => (
                    <tr key={r.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        ...
                        <p className="text-xs text-slate-500">{ROLE_LABEL[r.usuarioRole] ?? r.usuarioRole}{r.usuarioJvpm ? ` · ${r.usuarioJvpm}` : ""}</p>
                      </td>
                      <td className="px-4 py-3">
                        <span className="inline-flex px-2 py-0.5 rounded-full text-xs font-medium border bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-700">
                          {CRITERIO_LABEL[r.criterio]}
                        </span>
                        <p className="text-xs text-slate-500 font-mono mt-1">{r.termino || "-"}</p>
                      </td>
                      <td className="px-4 py-3 text-slate-700 dark:text-slate-300">
                        {r.pacienteNombre || "-"}
                        <span className="block text-xs text-slate-500 font-mono mt-0.5">Exp. {r.expedienteConsultado}</span>
                      </td>
                      <td className="px-4 py-3">
                        {r.tieneIngresoActivo ? (
                          <>
                            <span className="inline-flex px-2 py-0.5 rounded-full text-xs font-medium border bg-emerald-50 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-900">
                              Activo
                            </span>
                            {r.servicioActual && (
                              <span className="block text-xs text-slate-500 mt-1">
                                {r.servicioActual}{r.camaActual ? ` · Cama ${r.camaActual}` : ""}
                              </span>
                            )}
                          </>
                        ) : (
                      ...
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-400 tabular-nums">{r.totalEstancias}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              {filtrados.map(r => (
                <div key={r.id} className="p-4 space-y-2">
                  ...
                  <p className="text-sm text-slate-700 dark:text-slate-300">
                    {r.pacienteNombre || "Sin nombre"} · Exp. <span className="font-mono font-semibold">{r.expedienteConsultado}</span>
                  </p>
                  <p className="text-xs text-slate-500">
                    {ROLE_LABEL[r.usuarioRole] ?? r.usuarioRole}
                    {" · "}
                    {r.tieneIngresoActivo ? `Activo${r.servicioActual ? ` (${r.servicioActual})` : ""}` : "Sin ingreso activo"}
                    {" · "}
                    {r.totalEstancias} estancia{r.totalEstancias === 1 ? "" : "s"}
                  </p>
                </div>
              ))}
            </div>
