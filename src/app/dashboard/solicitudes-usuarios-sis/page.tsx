...
function numeroDocumento(solicitud: Solicitud) { return solicitud.numeroDocumento || solicitud.dui || "—"; }
function etiquetaDocumento(solicitud: Solicitud) { return `${TIPOS_DOCUMENTO_SIS.find((tipo) => tipo.value === solicitud.tipoDocumento)?.label || "DUI"}: ${numeroDocumento(solicitud)}`; }
...
export default function SolicitudesUsuariosSisPage() {
  ...
  useEffect(() => {
    if (puedeAcceder) void cargar();
    if (profile?.role === "medico_licenciado_dimes") setFiltro("creado");
  }, [profile?.role, puedeAcceder]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return <div className="mx-auto max-w-6xl p-4 md:p-6">
    <div className="mb-6 flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl border border-cyan-200 bg-cyan-50 text-cyan-700 dark:border-cyan-900 dark:bg-cyan-950 dark:text-cyan-300"><ClipboardList size={19} /></span><div><h1 className="font-heading text-xl font-bold text-slate-900 dark:text-slate-100">Solicitudes de usuarios SIS</h1><p className="text-xs text-slate-500">{soloLectura ? "Consulta de usuarios SIS ya creados. Sin opciones de edición." : "Registro de solicitudes y seguimiento de creación en SIS."}</p></div></div><button onClick={() => void cargar()} className="rounded-lg border border-slate-300 px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800">Actualizar</button></div>
    {!soloLectura && <div className="mb-5 grid grid-cols-2 gap-3 md:grid-cols-4">{(["pendiente", "en_proceso", "creado", "rechazado"] as EstadoSolicitudSis[]).map((e) => <button key={e} onClick={() => setFiltro(e)} className={`rounded-xl border p-3 text-left transition ${estadoCls[e]} ${filtro === e ? "ring-2 ring-offset-1 ring-blue-500 dark:ring-offset-slate-950" : "opacity-80 hover:opacity-100"}`}><p className="text-xs">{ESTADO_SOLICITUD_SIS_LABEL[e]}</p><p className="mt-1 text-2xl font-bold">{conteo(e)}</p></button>)}</div>}
    <div className="mb-4 flex flex-col gap-2 rounded-xl border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-900 sm:flex-row"><div className="relative flex-1"><Search size={15} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /><input value={busqueda} onChange={(e) => setBusqueda(e.target.value)} className={`${inputCls} pl-9`} placeholder="Buscar por nombre, documento, cargo, servicio o especialidad..." /></div>{!soloLectura && <select value={filtro} onChange={(e) => setFiltro(e.target.value as EstadoSolicitudSis | "todas")} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-700 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200"><option value="todas">Todos los estados</option>{(["pendiente", "en_proceso", "creado", "rechazado"] as EstadoSolicitudSis[]).map((e) => <option key={e} value={e}>{ESTADO_SOLICITUD_SIS_LABEL[e]}</option>)}</select>}</div>
