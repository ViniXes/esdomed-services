...
let cacheBusquedas: BusquedaTelefono[] | null = null;
...
export default function HistorialBusquedasPage() {
  ...
  const [registros, setRegistros] = useState<BusquedaTelefono[]>(() => cacheBusquedas ?? []);
  const [cargando, setCargando] = useState(false);
  const [consultado, setConsultado] = useState(cacheBusquedas !== null);
  ...
  const consultar = async () => {
    ...
    try {
      ...
      const lista = snap.docs.map(d => ({ id: d.id, ...d.data() } as BusquedaTelefono));
      cacheBusquedas = lista;
      setRegistros(lista);
      setConsultado(true);
    } finally {
      setCargando(false);
