...
// porque el trámite es del comité, no del servicio de Psicología. Psicología y
// Trabajo Social apoyan el trámite, así que entran a estas vistas con su
// propio menú, para que el cruce entre áreas sea transparente:
...
// a ambas áreas los mismos permisos del módulo que al comité.
function ComiteLesionesContent({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (loading) return;
    if (!esComite && !esPsicologia && !esTS) router.replace("/login");
    else if (vetado) router.replace("/comite-lesiones");
  }, [loading, esComite, esPsicologia, esTS, vetado, router]);
  ...
  const roleLabel = esPsicologia ? "Psicología" : esTS ? "Trabajo Social" : "Comité de lesiones";
